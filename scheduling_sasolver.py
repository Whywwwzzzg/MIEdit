...
class SASolverScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if trained_betas is not None:
            ...
            self.betas = torch.linspace(root_start, root_end, num_train_timesteps) ** p
        ...
        # 额外保存一套 scaled_linear betas（专门用于 sample_xts_from_x0 反演）
        # self.betas_inversion = (
        #     torch.linspace(
        #         beta_start**0.5,
        #         beta_end**0.5,
        #         num_train_timesteps,
        ...
        root_start = beta_start ** (1.0 / p)
        root_end = beta_end ** (1.0 / p)
        self.betas_inversion = torch.linspace(root_start, root_end, num_train_timesteps) ** p
    ...
    def sa_to_flow_time(self, sa_sigma):
        """
        将当前 SA-Solver 的 sigma 值映射到 Flow Matching 的时间参数。
        """
        # 获取 Flow Matching 配置
        flow_config = {
            "use_dynamic_shifting": getattr(self.config, "use_dynamic_shifting", False),
            "mu": getattr(self, "mu", 1.0),  # 需要在 set_timesteps 中设置
...
def convert_sa_to_flow_time(sa_sigma, shift=3.0, scheduler_config=None):
        """
        将 SA-Solver 的 sigma 值映射到 Flow Matching 的时间参数。
        
        Args:
            sa_sigma (torch.Tensor or float): SA-Solver 的 sigma 值
            shift (float, optional): Flow Matching 的 shift 参数，SD3 推荐 3.0
            scheduler_config (dict, optional): 调度器配置，用于动态 shifting
        
        Returns:
            torch.Tensor or float: Flow Matching 对应的时间参数 t
        """
        # 1. 从 SA-Solver 的 sigma 计算 alpha_t
        # SA-Solver: sigma = sqrt((1-alpha)/alpha)
        # => alpha = 1/(sigma^2 + 1)
        alpha_t = 1 / ((sa_sigma**2 + 1) ** 0.5)
        
        # 2. 计算噪声水平（扩散过程中的噪声比例）
        # 在扩散模型中，noise_level = 1 - alpha_t
        noise_level = 1 - alpha_t
        
        # 3. 根据 Flow Matching 的配置应用时间映射
        if scheduler_config is not None and scheduler_config.get("use_dynamic_shifting", False):
            # 动态 shifting 需要 mu 参数
            ...
            if time_shift_type == "exponential":
                # 指数型动态 shifting: t' = exp(mu) / (exp(mu) + (1/t - 1)^sigma)
