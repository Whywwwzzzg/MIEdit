...
class SASolverScheduler(SchedulerMixin, ConfigMixin):
    ...
    Args:
        ...
        beta_schedule (`str`, defaults to `"linear"`):
            Legacy Diffusers configuration key selecting the schedule construction.
            `scaled_linear` activates the power-interpolated schedule.
    ...
    def __init__(
        ...
        use_flow_sigmas: Optional[bool] = True,
        ...
        if trained_betas is not None:
            ...
            # Power-interpolated schedule.
            p = 1.9  # Used only for diffusion-model sampling (e.g., PixArt).
            root_start = beta_start ** (1.0 / p)
            root_end = beta_end ** (1.0 / p)
            self.betas = (
                torch.linspace(
                    root_start,
                    root_end,
        ...
        if tau_func is None:
            if use_h_zero_cache:
                self.tau_func = lambda t: 0.8 if t < 150 else (0.8 if t <= 800 else 0.8)
            else:
                self.tau_func = lambda t: 0 if t < 150 else 0.8
        else:
            self.tau_func = tau_func
        self.use_h_zero_cache = use_h_zero_cache
        self.predict_x0 = algorithm_type == "data_prediction"
        self.lower_order_nums = 0
        self.last_sample = None
        self._step_index = None
        self._begin_index = None
        self._h_zero_noise_parts_pred = {}
        self._h_zero_noise_parts_corr = {}
    ...
    def check_h_zero(self, k: int) -> bool:
        """Return True when sigmas[k] ≈ sigmas[k+1], i.e. h ≈ 0 (identity step)."""
        if k + 1 >= len(self.sigmas):
            return False
        return (self.sigmas[k] - self.sigmas[k + 1]).abs() < 1e-8
    ...
    def set_timesteps(self, num_inference_steps: int = None, device: Union[str, torch.device] = None):
        ...
        # "linspace", "leading", "trailing" corresponds to annotation of Table 2. of
    ...
    def _convert_to_karras(self, in_sigmas: torch.Tensor, num_inference_steps) -> torch.Tensor:
        ...
        return sigmas
    def sa_to_flow_time(self, sa_sigma):
        """
        Map the current SA-Solver sigma value to the Flow Matching time parameter.
        """
        # Read the Flow Matching configuration.
        flow_config = {
            "use_dynamic_shifting": getattr(self.config, "use_dynamic_shifting", False),
            "mu": getattr(self, "mu", 1.0),  # Must be set in set_timesteps.
            "time_shift_type": getattr(self.config, "time_shift_type", "exponential"),
            "flow_shift": getattr(self.config, "flow_shift", 3.0)
        }
        # Apply the conversion.
        return convert_sa_to_flow_time(
            sa_sigma,
            shift=3,
            scheduler_config=flow_config if flow_config["use_dynamic_shifting"] else None
        )
        ...
        **kwargs,
        ...
        if self.config.algorithm_type in ["data_prediction"]:
            if self.config.prediction_type == "epsilon":
                ...
                # sa_sigma = self.sigmas[k]
        
                # Map to the Flow Matching time parameter.
                # flow_t = self.sa_to_flow_time(sa_sigma)
                
                # print(f"Step {k}: SA sigma={sa_sigma:.4f} -> Flow time={flow_t:.4f}")
                
                # Compute the x0 prediction using the mapped time.
                # x0_pred = sample - flow_t * model_output
                sigma_t = self.sigmas[k]
    ...
    def sample_xts_from_x0(self, x_0, num_inference_steps=50, generator=None):
        """Flow Matching forward: x_t = (1-sigma)*x_0 + sigma*noise, vectorised."""
        _, channels, height, width = x_0.shape
        device = x_0.device
        dtype = x_0.dtype

        noise = torch.randn(x_0.shape, generator=generator, device=device, dtype=dtype)

        t = torch.linspace(0, 1, num_inference_steps + 1, device=device, dtype=dtype)
        shift = self.config.flow_shift
        sigma = shift * t / (1 + (shift - 1) * t)  # (N+1,)

        # Broadcast: sigma (N+1,1,1,1) * x_0 (1,C,H,W) -> (N+1,C,H,W)
        sigma_4d = sigma.view(-1, 1, 1, 1)
        xts = (1 - sigma_4d) * x_0 + sigma_4d * noise

        xts[-1] = noise
        return xts
    def inverter_predict(
        self,
        rela,
        model_output_list,
        *args,
        ...
        **kwargs,
        ...
        # print(f"mosel_output=(x_{timestep}-{sigma_t}*model_output)/{alpha_t}")
        # model_output = (sample - sigma_t * model_output) / alpha_t
        ...
        if self.predict_x0:
            ...
            # print(f"x_s=(x_{timestep}-{(1 + tau**2)* sigma_t* torch.exp(-(tau**2) * lambda_t)* gradient_coefficients[0]}*modeloutput:{model_output.mean()}-{sigma_t * torch.sqrt(1 - torch.exp(-2 * tau**2 * h)) }*noise({noise.mean()}))/{(torch.exp(-(tau**2) * h)*(sigma_t / sigma_s0))}")
            # # x_s = (x-(gradient_part + noise_part))/(torch.exp(-(tau**2) * h)*(sigma_t / sigma_s0))
            noise_part = x - (torch.exp(-(tau**2) * h) * (sigma_t / sigma_s0) * rela +  gradient_part )

            if tau > 0 and h.abs() > 1e-8:
                noise = noise_part / (sigma_t * torch.sqrt(1 - torch.exp(-2 * tau**2 * h)))
            else:
                noise = torch.zeros_like(noise_part)
                if self.use_h_zero_cache and h.abs() <= 1e-8:
                    self._h_zero_noise_parts_pred[k] = noise_part
            x_t = torch.exp(-(tau**2) * h) * (sigma_t / sigma_s0) * rela + gradient_part + noise_part
            # x_s = (x-(gradient_part + noise_part))/(torch.exp(-(tau**2) * h)*(sigma_t / sigma_s0))
            # x_t = torch.exp(-(tau**2) * h) * (sigma_t / sigma_s0) * xs + gradient_part + noise_part
            # x_s = torch.exp(-(tau**2) * h) * (sigma_t / sigma_s0) * x + gradient_part + noise_part
        # else:
        #     x_s = (alpha_t / alpha_s0) * x + gradient_part + noise_part

        # x_s = x_s.to(x.dtype)
        return rela,noise,x_t
    ...
    def stochastic_adams_bashforth_update(
        ...
        **kwargs,
        ...
        # print(f"samplingk:{self.step_index}")
        ...
        if self.predict_x0:
            si = self.step_index
            if self.use_h_zero_cache and h.abs() <= 1e-8 and si in self._h_zero_noise_parts_pred:
                noise_part = self._h_zero_noise_parts_pred.pop(si)
            else:
                noise_part = sigma_t * torch.sqrt(1 - torch.exp(-2 * tau**2 * h)) * noise
        else:
            noise_part = tau * sigma_t * torch.sqrt(torch.exp(2 * h) - 1) * noise
    ...
    def inverter_update(
        ...
        **kwargs,
        ...
        lambda_t = torch.log(alpha_t) - torch.log(sigma_t)
        lambda_s0 = torch.log(alpha_s0) - torch.log(sigma_s0)
        gradient_part = torch.zeros_like(last_sample)
        h = lambda_t - lambda_s0
        lambda_list = []
        for i in range(order):
            si = k+1 - i
        ...
        for i in range(order):
            if self.predict_x0:
                gradient_part += (
                    ...
                    * torch.exp(-(tau**2) * lambda_t)
                    * gradient_coefficients[i]
                    * model_output_list[zhengxu-2+(i)]
                )
              
            else:
                gradient_part += -(1 + tau**2) * alpha_t * gradient_coefficients[i] * model_output_list[zhengxu-1-(i)]
        ...
        if self.predict_x0:
            ...
            if tau > 0 and h.abs() > 1e-8:
                ...
                noise = torch.zeros_like(noise_part)
                if self.use_h_zero_cache and h.abs() <= 1e-8:
                    self._h_zero_noise_parts_corr[k] = noise_part

            xt2 = torch.exp(-(tau**2) * h) * (sigma_t / sigma_s0) * rela + gradient_part + noise_part
        else:
            # noise_part = tau * sigma_t * torch.sqrt(torch.exp(2 * h) - 1) * last_noise
            noise_part = x - (alpha_t / alpha_s0)*rela -  gradient_part
        # if self.predict_x0:
        #     noise_part = sigma_t * torch.sqrt(1 - torch.exp(-2 * tau**2 * h)) * last_noise
        # else:
        #     noise_part = tau * sigma_t * torch.sqrt(torch.exp(2 * h) - 1) * last_noise
    ...
    def stochastic_adams_moulton_update(
        ...
        **kwargs,
        ...
        if self.predict_x0:
            si = self.step_index
            if self.use_h_zero_cache and h.abs() <= 1e-8 and si in self._h_zero_noise_parts_corr:
                noise_part = self._h_zero_noise_parts_corr.pop(si)
            else:
                noise_part = sigma_t * torch.sqrt(1 - torch.exp(-2 * tau**2 * h)) * last_noise
        else:
            noise_part = tau * sigma_t * torch.sqrt(torch.exp(2 * h) - 1) * last_noise
    ...
    def invert(
        ...
        *args,
        ...
        use_corrector = k >= 0 and k < self.num_inference_steps - 1
        xt2=None
        xt=None
        if self.config.lower_order_final:
            this_predictor_order_inv = min(self.config.predictor_order, zhengxu)
            this_corrector_order_inv = min(self.config.corrector_order, zhengxu+ 1)
        else:
            this_predictor_order_inv = self.config.predictor_order
            this_corrector_order_inv = self.config.corrector_order
        
        self.this_predictor_order_inv = min(this_predictor_order_inv, k + 1)  # warmup for multistep
        self.this_corrector_order_inv = min(this_corrector_order_inv, k + 2)  # warmup for multistep
        ...
        # print(f"self.this_predictor_order_inv:{self.this_predictor_order_inv}")
        # print(f"self.this_corrector_order_inv:{self.this_corrector_order_inv}")
        assert self.this_predictor_order_inv > 0
        assert self.this_corrector_order_inv > 0

        self.last_sample = sample

        # print(f"timestep:{timestep}")
        current_tau = self.tau_func(xTtimestep.item())
        # current_tau =1
        ...
        #     sample=sample,
        #     noise=noise1,
        #     order=self.this_predictor_order_inv,
        #     tau=current_tau,
        #     timestep=timestep,
        # )
        rela,noise1,xt=self.inverter_predict(
        ...
        # print(f"xtmean:{xt.mean()},samplemean:{sample.mean()}")
        if use_corrector:
            ...
            #     tau=current_tau,
            # )
            latents,noise2,xt2=self.inverter_update(
                rela=rela,
                model_output_list=model_output_list,
                last_sample=xt,
                order=self.this_corrector_order_inv,
                k=k,
                zhengxu=zhengxu,
                tau=current_tau,
                timestep=timestep,
            ...
            # print(f"last_samplemean:{xt.mean()},xt2mean:{xt2.mean()}")
        # if self.lower_order_nums < max(self.config.predictor_order, self.config.corrector_order - 1):
        #     self.lower_order_nums += 1
        else:
            latents=rela
            noise2=noise1
        pre = xt if xt2 is None else xt2
        return rela,latents,noise1,noise2,pre
    ...
    def step(
        ...
        model_output = self.convert_model_output(model_output, sample=sample,timestep=timestep)
...
def convert_sa_to_flow_time(sa_sigma, shift=3.0, scheduler_config=None):
        """
        Map an SA-Solver sigma value to the Flow Matching time parameter.
        
        Args:
            sa_sigma (torch.Tensor or float): The SA-Solver sigma value.
            shift (float, optional): The Flow Matching shift; 3.0 is recommended for SD3.
            scheduler_config (dict, optional): Scheduler configuration used for dynamic shifting.
        
        Returns:
            torch.Tensor or float: The corresponding Flow Matching time parameter t.
        """
        # 1. Compute alpha_t from the SA-Solver sigma.
        # SA-Solver: sigma = sqrt((1-alpha)/alpha)
        # => alpha = 1/(sigma^2 + 1)
        alpha_t = 1 / ((sa_sigma**2 + 1) ** 0.5)
        
        # 2. Compute the noise level (the noise fraction in the diffusion process).
        # In diffusion models, noise_level = 1 - alpha_t.
        noise_level = 1 - alpha_t
        
        # 3. Apply time mapping based on the Flow Matching configuration.
        if scheduler_config is not None and scheduler_config.get("use_dynamic_shifting", False):
            # Dynamic shifting requires the mu parameter.
            
            mu = scheduler_config.get("mu", 1.0)
            time_shift_type = scheduler_config.get("time_shift_type", "exponential")
            
            if time_shift_type == "exponential":
                # Exponential dynamic shifting: t' = exp(mu) / (exp(mu) + (1/t - 1)^sigma)
                flow_t = math.exp(mu) / (math.exp(mu) + (1 / (noise_level + 1e-8) - 1) ** 1.0)
                # flow_t = noise_level
            elif time_shift_type == "linear":
                # Linear dynamic shifting.
                flow_t = mu / (mu + (1 / (noise_level + 1e-8) - 1) ** 1.0)
            else:
                raise ValueError(f"Unsupported time_shift_type: {time_shift_type}")
        else:
            # Standard shifted linear mapping.
            if shift == 1.0:
                flow_t = noise_level
            else:
                # shifted linear: t' = shift * t / (1 + (shift-1)*t)
                flow_t = shift * noise_level / (1 + (shift - 1) * noise_level)
        
        # 4. Clamp the result to the [0, 1] range.
        if isinstance(flow_t, torch.Tensor):
            flow_t = torch.clamp(flow_t, 0.0, 1.0)
        else:
            flow_t = max(0.0, min(1.0, flow_t))
