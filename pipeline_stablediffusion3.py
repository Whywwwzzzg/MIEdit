...
class SD3EditingPipeline(StableDiffusion3Pipeline):
    ...
    def generate_with_gt_mask_editing(
        ...
        max_single_k_mask_steps: int = 10,
        max_single_v_mask_steps: int = 10,
        ...
        xts = self.scheduler.sample_xts_from_x0(
            init_latents,
        ...
        final_mask_kv = None
        final_mask_cfg = None
        step_kv_masks: Dict[int, np.ndarray] = {}
        step_v_masks: Dict[int, np.ndarray] = {}
        ...
        for i, t in enumerate(inv_timesteps):
            ...
            if i in cfg_diff_set and do_cfg_src:
                ...
                if raw_diff is not None:
                    ...
                    abs_d = np.abs(raw_diff)
                    refined, otsu_n = _sigmoid_crf_otsu(abs_d)
                    if i <= max_single_v_mask_steps:
                        step_v_masks[i] = (refined > otsu_n).astype(np.float32)
                    if i <= max_single_k_mask_steps:
                        step_kv_masks[i] = self._morphological_close_mask(
        ...
        # Pre-convert per-step KV masks to GPU tensors for editing.
        step_kv_masks_gpu: Dict[int, torch.Tensor] = {
            k: torch.from_numpy(v).unsqueeze(0).unsqueeze(0).float().to(device)
            for k, v in step_kv_masks.items()
        }
        step_v_masks_gpu: Dict[int, torch.Tensor] = {
            k: torch.from_numpy(v).float().to(device) if isinstance(v, np.ndarray) else v.to(device)
            for k, v in step_v_masks.items()
        }
        ...
        for step_idx, t in enumerate(timesteps):
            ...
            if skip_transformer:
                ...
                kv_mask = None
                if should_use_kv:
                    if inv_step in step_kv_masks_gpu:
                        kv_mask = step_kv_masks_gpu[inv_step]
                    elif final_mask_kv is not None:
