...
DEFAULT_CLIP_MODEL_CANDIDATES = [
    "/data/disk2/haiyan/huggingface/hub/models--openai--clip-vit-large-patch14",
...
def _choose_clip_model_path(clip_model_path=None):
    candidates = []
    if clip_model_path:
        candidates.append(clip_model_path)
    env_path = os.environ.get("CLIP_MODEL_PATH") or os.environ.get("HF_CLIP_MODEL_PATH")
    if env_path:
        candidates.append(env_path)
    candidates.extend(DEFAULT_CLIP_MODEL_CANDIDATES)

    for candidate in candidates:
        if candidate == "openai/clip-vit-large-patch14":
            continue
        if Path(candidate).expanduser().exists():
            return _resolve_hf_snapshot_path(candidate)

    return clip_model_path or env_path or "openai/clip-vit-large-patch14"
...
class MetricsCalculator:
    def __init__(self, device, clip_model_path=None) -> None:
        self.device=device
        model_path = _choose_clip_model_path(clip_model_path)
        print(f"Loading CLIPScore from: {model_path}")
        self.clip_metric_calculator = CLIPScore(model_name_or_path=model_path).to(device)
