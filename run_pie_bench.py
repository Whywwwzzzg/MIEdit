...
from scheduling_MIEdit_SD3_5 import SASolverScheduler


METHOD_NAME = (
    "maxk_11_maxv_12_cfgdiff_8_9_10_11_12_13_"
    "layers_7_10_13_16_19_sgs_1p6_gs_3p6_steps_27_skip_7"
)
...
def parse_cfg_diff_steps(raw_steps: str | None):
    if not raw_steps:
        return None
    steps = [int(item) for item in raw_steps.replace(",", " ").split()]
    if not steps:
        raise ValueError("--cfg_diff_steps must contain at least one step")
    return steps
...
def evaluate_results(args):
    ...
    result_path = args.eval_result_path or os.path.join(
        os.path.dirname(target_path), "eval_maxk11_maxv12_cfgdiff_8_13.csv"
    )
    summary_path = args.eval_summary_path or os.path.join(
        os.path.dirname(target_path), "eval_maxk11_maxv12_cfgdiff_8_13_summary.csv"
    )
...
def main():
    ...
    parser.add_argument(
        '--target_path',
        type=str,
        default=os.path.join('camera_ready_results', METHOD_NAME),
    ...
    parser.add_argument('--num_inference_steps', type=int, default=27,
                        help='Number of inference steps (default: 27)')
    parser.add_argument('--guidance_scale', type=float, default=3.6,
                        help='Guidance scale for generation/editing (default: 3.6)')
    parser.add_argument('--source_guidance_scale', type=float, default=1.6,
                        help='Guidance scale for inversion (default: 1.6)')
    parser.add_argument('--skip_steps', type=int, default=7,
                        help='Number of initial steps to use inversion latents (default: 7)')
    parser.add_argument('--generator_seed', type=int, default=2,
                        help='Random seed for generator (default: 2)')
    ...
    parser.add_argument('--mask_feature_layers', type=str, default='7,10,13,16,19',
                        help='CFG-difference feature layers, e.g. "6,10,14,18,22"')
    parser.add_argument('--cfg_diff_steps', type=str, default='8,9,10,11,12,13',
                        help='CFG-difference steps used to build masks')
    parser.add_argument('--max_single_k_mask_steps', type=int, default=11,
                        help='Maximum inversion step that uses a per-step KV mask')
    parser.add_argument('--max_single_v_mask_steps', type=int, default=12,
                        help='Maximum inversion step that uses a per-step latent-blending mask')
    ...
    cfg_diff_steps = parse_cfg_diff_steps(args.cfg_diff_steps)
    if mask_feature_layers is not None:
        print(f"Using mask feature layers: {mask_feature_layers}")
    if cfg_diff_steps is not None:
        print(f"Using cfg_diff_steps: {cfg_diff_steps}")
    print(
        "Using per-step masks: "
        f"max_single_k_mask_steps={args.max_single_k_mask_steps}, "
        f"max_single_v_mask_steps={args.max_single_v_mask_steps}"
    )
    ...
    # Store output masks under target to keep them out of the working directory.
    ...
    for idx in range(start_idx, end_idx):
        ...
        # Mirror the output subdirectory layout for masks.
        ...
        try:
            ...
            # Create a new generator for each inference call to ensure reproducibility.
            ...
            result = pipe.generate_with_gt_mask_editing(
                ...
                cfg_diff_steps=cfg_diff_steps,
                mask_feature_layers=mask_feature_layers,
                max_single_k_mask_steps=args.max_single_k_mask_steps,
                max_single_v_mask_steps=args.max_single_v_mask_steps,
            ...
            if output_image is None:
                raise RuntimeError(
                    "The result does not contain generated_image; check the return format of "
                    "generate_with_gt_mask_editing."
                )
